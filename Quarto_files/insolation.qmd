...
In cyclostratigraphy it is common to refer to Milankovitch cycles to designate the orbital, precession and obliquity cycles. In fact, Milankovitch did not contributed that much to the identification and quantification of these cycles of astronomical theory, even though he provided a solid review at the time. What he did, by contrast, is to develop a good theory of the relationship between these cycles and climate, considering the changes in energy balance of the Earth and accounting for the reflection of sunlight by snow. 

A large part of his book "Canon of insolation" is devoted to the computation of insolation quantities: the _annual march of insolation_, which quantifies the distribution of insolation over latitudes and seasons, and the _secular march of insolation_ which determines of a given insolation quantity evolves over time. In this effort, he gave a particular importance to the definition and calculation of _caloric insolation_: the cumulated energy received at the top of the atmosphere over the half-part of the year that receives the largest part of it. Its computation can be a tad technical, but he considered this quantity to be the best predictor of accumulating effects, year after year, of insolation on snow and ice cover. Remember that the primary aim of Milankovitch was to provide a solid foundation to the theory that "ice-ages" are caused by the changes in insolation caused by the "movement of the poles", that is , the changes in precession and obliquity caused by planetary motion and luni-solar precession. 

The present document was written for the Coldigiocco summer school of cyclostratigraphy (Sept 2025) and stands as a first draft towards a one-stop, quick teaser about how insolation changes with orbital forcing, and what this implies for climate. We will use the `palinsol` package. 
...
orbital solutions. Work in progress, and feedback welcome! 

Please raise concerns, ask questions, or submit bug fixes or improvements is via the ![github](www.github.com/mcrucifix/palinsol) webpage of the project. 
...
though it is usual to divide this quantity by 86400s to obtain an _average daily insolation_, expressed in W/m$^2$. 
...
ecliptic by going in the upwards direction. By definition, at this very time, the Sun is aligned with the point of Aries $\gamma$. True solar longitude is an
angle to express the position of the Sun for an observer located on Earth. Now, based on that convention, you can see that from a _heliocentric_ point of 
...
For historical reasons, the package `palinsol` works with geocentric solar longitudes : rather than the position of the Earth around the Sun, we consider the position of the Sun for an Earth-bound observer. The conversion is straightforward:  $\varpi = \varpi^\prime + 180^\circ$ and 
...
Now, perhaps what interests you is to map $\nu$ (or, equivalently, $\lambda$) for any time of a calender. What is, for example, $\nu$ of August 23 ?
...
The equation linking $M$ and $\nu$ is transcendental but excellent approximations exist (not covered here, but standard, e.g. Brouwer and Clemence; Danjon...)
...
This equation satisfies a very basic fact well known since Kepler: the angular velocity of Earth ($\frac{\mathrm d\nu}{\mathrm d M}$) is inversely proportional to Earth-Sun distance $r$. 
...
This means that we have now two complementary ways to express the time of the year:

- The true solar longitude $\lambda$ which has clear square or flat angles at the season boundaries
- The mean  longitude $\ell$, zero at March equinox, and then progresses linearly as time goes? 
...
This extra shot is proportional to $r^{-2}$, thus proportional to $2e$. 

If $\varpi=90^\circ$, insolation will have a positive anomaly in June and a negative anomaly in December, both proportional to $e$, and little anomaly in March and Sept. 
...
considered negligible, but may be enough to influence significantly deep-ocean temperature and, perhaps, anoxic conditions. 

## The solar declination

The solar declination depicts the course of the Sun up and down the equator over time. It is straightforwardly linked with its longitude via a spherical triangle relationship:
...
We will visualise the distribution of insolation as a function of the time of the year and the latitude. First consider a reference orbit, assuming zero eccentricity and $\varepsilon=22.5^\circ$. That is: 
...
::: callout-tip
Observe that there are two local maxima in each Hemisphere : they both occur on the day of the Solstice, one at mid-latitudes sun high on horizon, at high latitudes (longest days). At the equator, the sun passes twice at the zenith, generating two local maxima over the year: one at the March equinox, and one at the Spring equinox. 
:::
...
Longitude_effect <- (Milankovitch(Orbit_DS) - Milankovitch(Orbit_JS))
...
```{r}
Ecc_effect <- (Milankovitch(Orbit_DS)  - Milankovitch(my_orbit))
...
Times_ky <- Times / 1e3         # times in kyr
...
for (i in seq(along = Orbits_constant_obl)) {
  Orbits_constant_obl[[i]]["eps"] <- 23.5 * pi / 180
}
...
## For visualisation purpose, we will, for each curve, - subtract the mean - divide by 3 - add the latitude in degrees.
## Given that I will need do do this again, I create a function


plot_my_series <- function(Latitudes, Series_of_Insolations, shrink = 4, ...) {
  # healthy check
  if (length(Latitudes) != length(Series_of_Insolations)) {
    stop("both inputs should have same length")
  }
  ...
  for (i in seq(along = Series_of_Insolations)) {
    ms <- mean(Series_of_Insolations[[i]])
    transform <- function(series) {
      (series - ms) / shrink + Latitudes[[i]]
    }
    sernor <- transform(Series_of_Insolations[[i]])
    lines(Times / 1000, sernor)

    lab <- ifelse(
      Latitudes[[i]] > 0,
      sprintf("%d N", Latitudes[[i]]),
      sprintf("%d S", -Latitudes[[i]])
    )
    if (Latitudes[[i]] == 0) lab <- "Eq."
    axis(2, at = c(Latitudes[[i]]), labels = c(lab))

    rmin <- (ceiling(min(Series_of_Insolations[[i]]) / 30)) * 30
    rmax <- (floor(max(Series_of_Insolations[[i]]) / 30)) * 30
    if (rmin > rmax) rmax = rmin
    axis(4, at = transform(c(rmin, rmax)), labels = c(rmin, rmax))
  }
}
...
### Obliquity on annual mean insolation
...
for (i in seq(along = Orbits_zero_ecc)) {
  Orbits_zero_ecc[[i]]["ecc"] <- 0
}
...
It is possible to construct a considerable number of insolation quantities: monthly insolation, seasonally-averaged insolation, seasonally-integrated (which is not the same thing), annual mean insolation, averaged or not over a range of latitudes...

It turns out that _most_ (but not all) insolation quantities can be expressed as a linear combination of obliquity, precession and co-precession parameters. This as an immediate implacation as to their spectral content. To see this, let's first consider one example and work out its spectrum. The classical example that serves as our basis is insolation on the day of the June solstice at 65$^\circ$ North. It can be computed with palinsol: 


```{r}
JS_Insol <-
    sapply(Orbits, function(orbit) Insol(orbit, long = 90 * pi / 180, lat = 65 * pi / 180))
# convert into a time series object
JS_Insol <- ts(JS_Insol, start=-1000)
plot(Times_ky, JS_Insol)
```

Now, we construct the three constitutive elements of this insolation quantity: precession, co-precession, obliquity and eccentricity.  We normalise them. 

```{r}
obliquity_norm <- ts(scale(sapply(Orbits, function(orbit) orbit['eps'])), start=-1000)
ecc_norm <- ts(scale(sapply(Orbits, function(orbit) orbit['ecc'])), start=-1000)
precess_norm <- ts(scale(sapply(Orbits, function(orbit) orbit['ecc'] * sin(orbit['varpi']) )), start=-1000)
coprecess_norm <- ts(scale(sapply(Orbits, function(orbit) orbit['ecc'] * cos(orbit['varpi']) )), start=-1000)
```

Now we want to express our insolation quantity as a linear combination of all three quantities. This is a mere linear regression problem, and `R` has built-in features to manage this: 

```{r}
decomp <- lm(scale(JS_Insol) ~ obliquity_norm + precess_norm + coprecess_norm + ecc_norm)
print (decomp$coefficients)
print (anova(decomposition))
```

As we see it, this June solstice insolation is fully explained (no residual) by this combination, which is dominated by precession (82% of variance), obliquity (17% of variance), and eccentricity (1 %). 

The 1%-eccentricity is related to the effect of eccentricity on the mean Earth-Sun distance; a very small effect indeed. 

We expect these ratios to be reflected in the spectrum. 


The respective ratios of precession and obliquity depend on the type of insolation and latitude. As a rule of thumb: 

- Obliquity becomes (often) more important when considering insolation averaged over a period of time (exceptions exist). For example, monthly mean insolation has more obliquity than insolation on the day of Summer solstice; 
- It also becomes more important at times of the year that are closer to the polar night (e.g. winter insolation at 60 $^\circ$ North
- Precession cancels (!) when considering insolation integrated over specific true solar longitudes (e.g. integrated over the astronomical summer), or, by extension, over the full year. 

Consider this example: 

TODO: INSOLATION OVER THE WHOLE SUMMER


- Eccentricity only shows up when all other effects are cancelled, for example if we consider globally averaged insolation, oven the whole year. 
...
- Precession tends to dominate obliquity on daily insolation  at most latitudes, except near the polar night. 
...
Caloric insolation ... [here to develop] 
...
Eccentricity only appears in insolation spectra as a tiny peak, associated with the effect on globally averaged insolation (the $\sqrt{1-e^2}$) effect, which is much smaller than effects associated with the redistributions over seasons and latitudes. 
