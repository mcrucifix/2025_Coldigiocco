...
bibtex:
	@if [ -e $(PDF_DIR)/$(JOBNAME)_handouts.aux ]; then \
	  echo "Running bibtex on $(JOBNAME)_handouts..."; \
	  bibtex $(PDF_DIR)/$(JOBNAME)_handouts; \
	fi
	@if [ -e $(PDF_DIR)/$(JOBNAME)_presentation.aux ]; then \
	  echo "Running bibtex on $(JOBNAME)_presentation..."; \
	  bibtex $(PDF_DIR)/$(JOBNAME)_presentation; \
	fi
